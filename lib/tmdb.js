...
export async function getFilmById(id, lang = "en") {
  ...
  const keywordEntries = detailsEn.keywords?.keywords || [];
  const keywords = keywordEntries.map((k) => k.name);
  const keywordIds = keywordEntries.map((k) => k.id);
...
export async function buildCandidatePool(genre_ids = [], year_from, year_to, era, maxFilms = MAX_POOL_FILMS) {
  ...
  const baseParams = {
    ...
    // Phase 2b: keep low-rated films out of the pool entirely.
...
export async function getInputFilmSignals(filmIds = []) {
  ...
  const keywordNames = new Map(); // keyword id -> name, for human-readable match reasons
  ...
  return { directors, keywordIds, keywordNames, decades };
}
    ...
    try {
      ...
      for (const movie of data?.results ?? []) {
        ...
        if (existing) {
          ...
          similarityMap.set(movie.id, { ...mapBasicFilm(movie), similarityScore: 1, sourceIds: [id] });
        }
      }
    } catch {
